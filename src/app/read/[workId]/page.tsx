...
function ReadingContent({ workId }: { workId: string }) {
  ...
  const renderChar = (ch: string, idx: number, sentence: (typeof sentences)[0]) => {
    ...
    const rubyContent = (
      <span className="inline-block text-center mx-[0.5px]">
    ...
    if (charData) {
      return (
        <CharTooltip
          key={idx}
          character={charData.traditional}
    ...
    // Character not in dictionary — render invisible placeholder ruby to keep
    // vertical alignment consistent with annotated characters in the same line
    return (
      <span key={idx} className="inline-block text-center mx-[0.5px]">
        {(toggles.pinyin || toggles.hanViet) && (
        ...
        {sentences.map((sentence, sIdx) => {
          ...
          return (
            <div key={sentence.id} className={showNewParagraph ? "mt-8" : "mt-4"}>
              {/* Main text */}
              <div className={`flex flex-wrap items-end tracking-wide leading-loose relative ${fontClass}`}>
                {toggles.annotations &&
